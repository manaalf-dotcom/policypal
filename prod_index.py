...
import core  # 复用你的：parse_pdf_to_pages/chunk_text_by_tokens/embed_texts_openai
from policy_paths import COMPARE_DIR

# 可选：BM25（建议 requirements 加 rank-bm25）
try:
...
def _tokenize(text: str) -> List[str]:
    # 简单 tokenizer，production 可换 spaCy/MeCab 等
...
def build_policy_index(
    ...
    # Step3: folder -> chunks json (用你的 core 能直接 ingest 任意 input_dir/output_path)
    ...
    for c in chunks:
        ...
        metadatas.append({
            ...
            # 生产里建议加：section_hint/extract_method
    ...
    if _HAS_BM25:
        ...
        bm25 = {
            "enabled": True,
            "tokenized": tokenized,  # 简化起见存 token；生产可只存 vocab + doc freq
        }
    else:
        bm25 = {"enabled": False}
